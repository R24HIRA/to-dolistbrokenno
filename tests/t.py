...
if __name__ == "__main__":
    ...
    df["instr_type"] = np.where(
        ...
        np.where(
            ...
            np.where(
                ...
                "Cash Transfer",
                np.where(
                    df["instrument"].str.contains("XCF"),
                    "FX Forward",
                    df["instr_type"]
                ),
            ),
    ...
    (business_date, inst_type, trade_id, maturity_date, book, sys_source, ccy, bkt, pv01)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    ...
    criteria = f"""
    business_date < '{past31_date:%Y-%m-%d}'
    """
    delete_from_db(table_name="bahamas_atom_pv01", condition=criteria, env="QA", fast=False)
    ...
    if LAMP_UPLOAD:
        ...
        lamp_upload(env, date, 12367, float(dl_VaR))  # fixed duplicate ID
